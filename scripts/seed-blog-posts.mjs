// Seeds LoogoNews blog posts as drafts for human review before publishing.
// Run with: npm run db:seed-posts
...
const posts = [
  {
    ...
    excerpt: 'Every unanswered call is a lead calling your competitor next — here is the fix that costs nothing to run.',
    tags: 'Missed Call Text-Back, HVAC, Lead Follow-Up',
    read_time: 4,
    content: `Last July, a homeowner's AC quit on a 97-degree Tuesday. She called three HVAC companies in eleven minutes. Two went to voicemail. One didn't pick up either — but texted her back in under a minute: "Sorry we missed your call — this is Coastline Air. What's going on with your system?" She replied with her address before the other two companies called her back.
...
Fix the response time before you fix anything else in your marketing. A better ad that feeds a slow callback is just a faster way to lose the same leads.`,
  },
  {
...
Look at your form submissions from the last two weeks. For each one, find the timestamp of the actual first contact — not when it was assigned, when the patient heard back. If most of those gaps are measured in hours, that's not a marketing problem. That's the fix that will move your numbers before anything else you try.`,
  },
  {
    title: 'The Google Review System Most Salon Owners Never Build',
    slug: 'salon-google-review-system',
    excerpt: 'Asking for reviews by hand doesn’t scale past your fifth client of the day — here is what does.',
    tags: 'Reputation Management, Salons & Spas, Google Reviews',
    read_time: 3,
    content: `A stylist finishes a color correction that took three hours and left a client thrilled enough to post a before-and-after on her own Instagram. Nobody asks her for a Google review. She means to leave one. She doesn't, because by the time she's home, she's onto the next thing.
...
Time your next ten review requests against the actual appointment end time, not checkout time. If most go out same-day but hours later, tighten that window first. The ask itself rarely needs work. When it goes out does.`,
  },
  {
    title: 'The First Five Minutes After a Lead Fills Out Your Form Decide the Deal',
    slug: 'real-estate-lead-response-time',
    excerpt: 'Real estate leads go cold fast — the agent who calls first usually wins, not the agent with the better listing photos.',
    tags: 'Lead Follow-Up, Real Estate, Speed to Lead',
    read_time: 3,
    content: `A buyer scrolling listings at 9pm requests info on three properties, each from a different agent's site. Two agents call the next morning. One texts within four minutes: "Hi — saw you're interested in the Maple St listing, it's still available, want to see it this weekend?"
...
Pull your last 20 leads and time the gap between form submission and first real contact — not assignment, contact. If the gap is measured in hours rather than minutes, that's the leak. Fix that before spending another dollar on lead generation.`,
  },
  {
    title: 'No-Shows Aren’t a People Problem — They’re a Systems Problem',
    slug: 'reduce-no-shows-appointment-reminders',
    excerpt: 'A single reminder text the night before does more for your schedule than any policy about cancellation fees.',
    tags: 'Booking & Scheduling, No-Show Reduction, Automation',
    read_time: 3,
    content: `An electrician books six appointments for a Thursday. Two people don't answer the door. Neither had bad intentions — one forgot, the other double-booked without realizing the conflict until the truck was already outside. Two no-shows on a six-job day isn't a scheduling problem. It's a third of the day's revenue, gone, with the truck already burning gas to get there.
...
Look at your last month of appointments and count the no-shows. Multiply that by your average ticket. That number is what a two-text reminder sequence is worth to you before you touch anything else in your scheduling process.`,
  },
];
...
async function seed() {
  ...
  for (const post of posts) {
    const [row] = await sql`
      ...
      VALUES (${post.title}, ${post.slug}, ${post.excerpt}, ${post.content}, ${post.tags}, 'draft', 'David Selva', ${post.read_time})
